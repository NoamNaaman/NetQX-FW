...
//u8 rdr4_change_state[MAX_DOORS] = { 0,0 };
...
//=============================================================================================
void rdr4_init_system(void)
  {
  rdr4_q_pace = 0;
  for (u32 idx = 0; idx < MAX_DOORS; idx++)
    {
    rdr4_reply_idx[idx] = 0;
    rdr4_wait_answer[idx] = 0;
    rdr4_rx_state[idx] = 0;
    rdr4_receiving[idx] = 0;
    rdr4_no_response[idx] = 0;
    }
  init_rdr4();
  }
...
//  enable_interrupts(int_TIMER1); // enable transmit timer interrupt
  }
...
void rdr4_process_reply(u32 reader)
  {
  u32 key;
  rdr4_reader_responded[reader] = true;
...
void rdr4_receive_bit(void)
  {
  u32 pin_state = 0;
  TIM15->ARR = bit_time; // 1 bit
  TIM15->CNT = 0;
  __HAL_TIM_CLEAR_FLAG(&htim15, TIM_FLAG_UPDATE);
  
  output_high(DEBUG_OUT);
  switch (rdr4_current_reader)
    {
    case 0: 
...
void rdr4_init_char_rx(u32 reader)
  {
  output_high(DEBUG_OUT);
//  disable_interrupt_pin(RDR4_EXTI_RX1);
//  disable_interrupt_pin(RDR4_EXTI_RX2);
//  disable_interrupt_pin(RDR4_EXTI_RX3);
//  disable_interrupt_pin(RDR4_EXTI_RX4);
  disable_interrupt_pin(RDR4_EXTI_RX1|RDR4_EXTI_RX2|RDR4_EXTI_RX3|RDR4_EXTI_RX4);

  __HAL_TIM_CLEAR_FLAG(&htim15, TIM_FLAG_UPDATE);
  __HAL_TIM_ENABLE_IT(&htim15, TIM_IT_UPDATE);
  HAL_NVIC_EnableIRQ(TIM15_IRQn);
  TIM15->ARR = bit_and_half; // 1.5 bits
  TIM15->CNT = 0;
