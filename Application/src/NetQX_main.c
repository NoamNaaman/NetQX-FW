...
void read_global_setup(void)
  {
//  SETUP_RECORD global_setup2;
//  SETUP_RECORD global_setup3;
//  return;
//  read_ext_eeprom(EE_GST, ADDR_SETUP,    (u8 *)&global_setup, sizeof(SETUP_RECORD));
//  delay_ms(50);
//  read_ext_eeprom(EE_GST, ADDR_SETUP+15, (u8 *)&global_setup2, sizeof(SETUP_RECORD));
//  delay_ms(50);
//  read_ext_eeprom(EE_GST, ADDR_SETUP+39, (u8 *)&global_setup3, sizeof(SETUP_RECORD));
//  if (global_setup.create_seq != global_setup2.create_seq ||
//      global_setup3.create_seq != global_setup2.create_seq ||
//      global_setup.create_seq != global_setup3.create_seq)
//    {
//    if (global_setup.create_seq == global_setup2.create_seq)
//      global_setup3.create_seq = global_setup2.create_seq;
//    else if (global_setup3.create_seq == global_setup2.create_seq)
//      global_setup.create_seq = global_setup2.create_seq;
//    else
//      global_setup2.create_seq = global_setup.create_seq;
//    }
//  write_global_setup();
  }
...
void write_global_setup(void)
  {
  write_ext_eeprom(ADDR_SETUP,    (u8 *)&global_setup, sizeof(SETUP_RECORD));
  write_ext_eeprom(ADDR_SETUP+15, (u8 *)&global_setup, sizeof(SETUP_RECORD));
  write_ext_eeprom(ADDR_SETUP+39, (u8 *)&global_setup, sizeof(SETUP_RECORD));
  }
...
//  read_ext_eeprom(EE_LDR1, ADDR_DOORS + door * sizeof(DOOR_RECORD), (u8 *)&dr, sizeof(DOOR_RECORD));
//  read_ext_eeprom(EE_LDR1, ADDR_DOORS + door * sizeof(DOOR_RECORD), (u8 *)&dr, sizeof(DOOR_RECORD));
/*  crc = CalculateCRC((u8 *)&dr, sizeof(DOOR_RECORD)-2);
  if (crc != dr.ErrorCheck || (dr.ErrorCheck && dr.Flags == 0xFFFFFFFFL))
    {
    read_ext_eeprom(EE_LDR2, door * sizeof(DOOR_RECORD), &dr, sizeof(DOOR_RECORD));
    crc = CalculateCRC(&dr, sizeof(DOOR_RECORD)-2);
    if (crc == dr.ErrorCheck && dr.Flags != 0xFFFFFFFFL)
      write_ext_eeprom(ADDR_DOORS + door * sizeof(DOOR_RECORD), &dr, sizeof(DOOR_RECORD));
    }
*/
...
//    COM1_send_packet(0, MyAddress, 5);// REMOVE REMOVE REMOVE REMOVE
...
void set_system_data(void)
  ...
  ky.Flags = 0;
  ky.al[0] = 1;
  ky.create_seq = global_setup.create_seq;
  DB_add_key(&ky);
  }
*/
