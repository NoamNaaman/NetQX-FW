...
bool allowed_to_open(u8 source, KEY_RECORD *key_rec)
  ...
  to   = key_rec->ToDate & 0x3FFF;//30000
...
//--------------------------------------------------------------------------
/*
u16 search_key_cache(u32 key, KEY_RECORD &key_rec)
  {
  u16 index;
  u32 *kp;
  kp = &kcode_cache[0];
  for (index = 0; index < KEY_CACHE_SIZE; index++, kp++)
    if (key == *kp)
      {
      memcpy(&key_rec.ID, &key_cache[index], sizeof(CACHE_RECORD));
      key_rec.Code = *kp;
      return ++index;
      }
  return 0;
...
void process_key(u8 source, u32 key, u16 special)
  ...
  bool personal;
  u16 mask, current_count, threshold, single_door, door = 0;
...
//  goto exec_unlock;
  ...
  single_door = ((doors[source].Flags & LFLAG_2RDR_SINGLE_DOOR) != 0) || (analog_values[ANALOG_SINGLE_DOOR] < 500);
...
//    return;
  ...
  key_rec.ID = 1;
  key_rec.PIN = 0x4FD1;
  key_rec.Flags = KFLAG_Master;
  key_rec.al[0] = 1;
  key_rec.FmDate = 111;
  key_rec.ToDate = 4000;
//  key_rec.create_seq = 0;
#else
...
tag_approved:
      if (test_door_flag(source, LFLAG_EXIT))                           // is this door an exit door?
        { // this is an exit                                                            // YES.
